...
/// Struct Config holds a list of read-in endpoints
...
impl Config {
    /// Default constructor (empty list) for Config struct
    pub fn make_default() -> Config {
        Config { hard_coded_contacts: vec![] /* No hardcoded endpoints */ }
    }
}

/// Reading a config file, raising an error if access failed
...
/// Creating a config file in a best-effort - we don't care about the result.
...
mod test {
    ...
    fn read_config_file_test() {
        ...
        for _ in 0..10 {
            let random_contact = ::endpoint::random_endpoint();
        ...
        let path_buf =  match super::write_config_file(Some(hard_coded_endpoints)) {
            Ok(result) => result,
            Err(err) => panic!("failed write config file with error {:?}", err),
        };
